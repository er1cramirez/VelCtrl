...
class VirtualCtrl {
    ...
    public:
        ...
        void process(flair::core::Quaternion &refOrientation, flair::core::Vector3Df &refOmega, float &thrust,
            ...
            flair::core::Quaternion refQuat;
            refQuat.q0 = u_3 * cosf(psi_d / 2) / 2;
            refQuat.q1 = (-uu.x * sinf(psi_d / 2) + uu.y * cosf(psi_d / 2)) / u_3;
            refQuat.q2 = (-uu.x * cosf(psi_d / 2) - uu.y * sinf(psi_d / 2)) / u_3;
            refQuat.q3 = sinf(psi_d / 2) * u_3 / 2;
            // Calculate desired angular velocity
            flair::core::Vector3Df omega;
            omega.x = -uup.x * sinf(psi_d) + uup.y * cosf(psi_d) + uup.z * (uu.x * sinf(psi_d) - uu.y * cosf(psi_d)) / (1 - uu.z);
            omega.y = -uup.x * cosf(psi_d) - uup.y * sinf(psi_d) + uup.z * (uu.x * cosf(psi_d) + uu.y * sinf(psi_d)) / (1 - uu.z);
            omega.z = psi_dot_d - (-uu.x * uup.y + uu.y * uup.x) / (1 - uu.z);
            // Set thrust and quaternion
            thrust = -norm;
            refOrientation = refQuat;
            refOmega = omega;
