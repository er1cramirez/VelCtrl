...
protected:
    double dt_ = 0.01;
    double mg_ = 9.81;  // Default mass * gravity
...
public:
    ...
    void setParam(const std::string& name, const Eigen::Vector3d& values) override {
        if (name == "kv") {
            ...
            BaseController::setParam(name, values);
        }
    }
...
public:
    ...
    void setParam(const std::string& name, double value) override {
        if (name == "window_size") {
            ...
            BaseEstimator::setParam(name, value);
        }
    }
...
public:
    ...
    void setParam(const std::string& name, const Eigen::Vector3d& values) override {
        if (name == "lambda") {
            ...
            BaseEstimator::setParam(name, values);
        }
    }
...
public:
    ...
    Eigen::Vector3d getEstimatedVelocity() const {
        if (estimator_) {
            return estimator_->getVelocity();
        }
        return Eigen::Vector3d::Zero();
    }
    
    Eigen::Vector3d getEstimatedAcceleration() const {
        if (estimator_) {
            return estimator_->getAcceleration();
        }
        return Eigen::Vector3d::Zero();
    }
...
Vector3Df RobustVelCtrl::getEstimatedVelocity() const {
    return fromEigen(pImpl->getEstimatedVelocity());
}

Vector3Df RobustVelCtrl::getEstimatedAcceleration() const {
    return fromEigen(pImpl->getEstimatedAcceleration());
}